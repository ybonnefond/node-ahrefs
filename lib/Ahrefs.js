...
Ahrefs.prototype._exec = function(query, callback, method) {
  ...
  var error = this._validate(query);
 ...
 * @private
 * @param {Query} query - Query to execute
 * @return {ValidationError|Error|null}
 */
Ahrefs.prototype._validate = function (query) {
  // Throw Error if the mandatory fields are not set
  if (!query.params.from) return new ValidationError('from', 'Missing `from` parameter');
  try {
